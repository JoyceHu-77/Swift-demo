...
public class GameScene: SKScene {
    ...
    public override func didMove(to view: SKView) {
        ...
        let j = states[0] as! JumpingState
        j.textures = (0..<1).map({ "jump\(playerNumber)/\($0)" }).map(SKTexture.init)
        let w = states[1] as! WalkingState
        w.textures = (0..<3).map({ "player\(playerNumber)/\($0)" }).map(SKTexture.init)
        let i = states[2] as! IdleState
        i.texture = SKTexture(imageNamed: "player\(playerNumber)/0")

        playerStateMachine = GKStateMachine(states: states)
        
        playerStateMachine.enter(IdleState.self)
        
        // Hearts
        heartContainer.position = CGPoint(x: -450, y: 250)
        heartContainer.zPosition = 5
        cameraNode?.addChild(heartContainer)
        fillHearts(count: 3)
        
        scoreLabel.position = CGPoint(x: (cameraNode?.position.x)! + 450, y: 250)
...
extension GameScene {
    public override func update(_ currentTime: TimeInterval) {
        ...
        if score >= 2 {
                   let nextLevel = GameScene(fileNamed: "ShowTheKnowledge\(playerNumber)")
